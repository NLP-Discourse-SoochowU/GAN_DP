...
from util.rst_utils import get_edus_info, get_one_edu_info
...
class Builder:
    ...
    def extent_trees(self):
        train_trees = load_data(RST_TRAIN_TREES_RST)
        train_t = load_data(RST_TRAIN_TREES)
        print(len(train_t[0].edus), train_trees[0].pre_traverse())
        print(len(train_t[1].edus), train_trees[1].pre_traverse())
        input()
        dev_trees = load_data(RST_DEV_TREES_RST)
        ext_trees = train_trees + dev_trees
        # EDU 加载
        with open(RST_EDUs_google_tr, "r") as f:
            ch_en_edus_tr = f.readlines()
        with open(RST_EDUs_google_de, "r") as f:
            ch_en_edus_de = f.readlines()
        ch_en_edus = ch_en_edus_tr + ch_en_edus_de
        result_trees_rst = []
        result_trees = []
        for tree_ in ext_trees:
            self.extent_one(tree_, ch_en_edus)
            result_trees_rst.append(tree_)
            result_trees.append(tree_obj(tree_))
        save_data(result_trees_rst, EXT_TREES_RST)
        save_data(result_trees, EXT_TREES)

    def extent_one(self, root, ch_en_edus):
        if root.left_child is not None:
            self.extent_one(root.left_child, ch_en_edus)
            self.extent_one(root.right_child, ch_en_edus)
        else:
            # edu
            tmp_edu = ch_en_edus.pop(0)
            edu_info = get_one_edu_info(root.temp_edu, tmp_edu, self.nlp)
            edu_txt, edu_ids, pos_ids, edu_emlo = edu_info
            root.temp_edu = edu_txt
            root.temp_edu_ids = edu_ids
            root.temp_pos_ids = pos_ids
            root.temp_edu_emlo_emb = edu_emlo
