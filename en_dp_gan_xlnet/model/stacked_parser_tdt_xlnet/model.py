...
class PartitionPtr(nn.Module):
    ...
    def init_session(self, edus, model_xl=None, tokenizer_xl=None):
    ...
    def encode_edus1(self, e_inputs, model_xl, tokenizer_xl):
        ...
        e_x = torch.zeros([1, XLNET_SIZE]).cuda(CUDA_ID)  # pad
        edu_encoded_x = None
        for e_in_edus in e_txt:
            ...
            for idx in range(max_edu_len):
                if idx < len(e_in_edus):
                    edu__ = e_in_edus[idx]
                    input_ids = torch.tensor([tokenizer_xl.encode(edu__, add_special_tokens=True)]).cuda(CUDA_ID)
