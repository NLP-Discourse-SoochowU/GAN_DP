...
def numericalize(dataset, word_vocab, pos_vocab, nuc_label, rel_label):
    ...
    for paragraph in filter(lambda d: d.root_relation(), chain(*dataset)):
        ...
        for edu in edus:
            ...
            tmp_edu = "".join(edu.words)
            edu_pos_ids = [pos_vocab[pos] for pos in edu.tags]
            encoder_inputs.append((edu_word_ids, edu_pos_ids, tmp_edu))
...
def gen_graph(splits):
    ...
    for idx_ in range(split_num):
        ...
        # print("start: ", start, "split: ", split, "end: ", end)
...
def gen_batch_iter(instances, batch_size, use_gpu=False):
    ...
    while offset < num_instances:
        ...
        for encoder_inputs, decoder_inputs, pred_splits, pred_nucs, pred_rels in batch:
            ...
            for edu_word_ids, _, _ in encoder_inputs:
        ...
        for batchi, (encoder_inputs, decoder_inputs, pred_splits, pred_nucs, pred_rels) in enumerate(batch):
            disc_edus = []
            for edui, (edu_word_ids, edu_pos_ids, edu_txt) in enumerate(encoder_inputs):
                disc_edus.append(edu_txt)
        ...
        if use_gpu:
            ...
            d_g_s = d_g_s.cuda()
            d_g_nr = d_g_nr.cuda()
        yield (e_input_words, e_input_poses, e_masks, e_boundaries, edus_txt), (d_inputs, d_masks), \
              (d_outputs, d_output_nr, d_g_s, d_g_nr, l_x)
        offset = offset + batch_size


def parse_and_eval(dataset_val, model, tokenizer_xl, model_xl):
    model.eval()
    model_xl.eval()
    ...
    parses = []
    for edus in strips:
        parse = parser.parse(edus, tokenizer_xl=tokenizer_xl, model_xl=model_xl)
        parses.append(parse)
...
def main(args):
    ...
    if EDU_ENCODE_V == 1:
        tokenizer_xl = AutoTokenizer.from_pretrained("hfl/chinese-xlnet-base")
        model_xl = AutoModel.from_pretrained("hfl/chinese-xlnet-base")
    else:
        tokenizer_xl = model_xl = None

    if model_xl is not None and not Finetune_XL:
        model_xl.eval()
    ...
    optimizer = optim.Adam([{'params': model.parameters()},
                            {'params': model_xl.parameters(), 'lr': 2e-5}], lr=args.lr, weight_decay=args.l2)
    best_model_score = 0.
    best_performance = 0.
    report_results = [0., 0., 0., 0.]
    rel_info, nucl_info = None, None
    for nepoch in range(1, args.epoch + 1):
        ...
        for nbatch, (e_inputs, d_inputs, grounds) in enumerate(batch_iter, start=1):
            ...
            if niter % args.validate_every == 0:
                num_instances, validate_scores_ = parse_and_eval(cdtb.validate, model, tokenizer_xl, model_xl)
                ...
                if model_score(validate_scores) > best_model_score:
                    ...
                    num_instances, test_scores_ = parse_and_eval(cdtb.test, best_model, tokenizer_xl, model_xl)
                    test_scores, info_ = test_scores_
                    if model_score(test_scores) > best_performance:
                        rel_info, nucl_info = info_
    ...
    print_("[Span: " + str(round(report_results[0], 4)) +
           ...
           " Full: " + str(round(report_results[3], 4)), args.log_dir)
    print_(obtain_rel_info(rel_info, nucl_info), args.log_dir)
    print_("=================== End ===================", args.log_dir)


def obtain_rel_info(rel_info, nucl_info):
    p_list, r_list, f_list = dict(), dict(), dict()
    if rel_info is None:
        return "None"
    else:
        rel_pre_true, rel_gold, rel_pre_all = rel_info
        nucl_pre_true, nucl_gold = nucl_info
        rels = rel_gold.keys()
        for rel_idx in rels:
        ...
        rel_info = rels + "\n" + p_info + "\n" + r_info + "\n" + f_info
        nucls = nucl_gold.keys()
        nucls_str = " ".join(nucls)
        results_nucl = []
        gold_nucls = []
        for nucl in nucls:
            tmp_acc = nucl_pre_true[nucl] / nucl_gold[nucl]
            gold_nucls.append(nucl_gold[nucl])
            results_nucl.append(tmp_acc)
        nucl_scores_str = " ".join([str(p_) for p_ in results_nucl]) + \
                          " GOLD NUCLs: " + " ".join([str(p_) for p_ in gold_nucls])
        nucl_info = nucls_str + nucl_scores_str
        return rel_info + "\n" + nucl_info
