...
class PartitionPtrParser:
    ...
    def parse(self, edus, ret_session=False, tokenizer_xl=None, model_xl=None):
        if len(edus) < 2:
            return Paragraph(edus)

        # TODO implement beam search
        session = self.init_session(edus, tokenizer_xl, model_xl)
    ...
    def init_session(self, edus, tokenizer_xl, model_xl):
        edu_words = [edu.words for edu in edus]
        edu_txts = ["".join(edu.words) for edu in edus]
        ...
        edu_encoded, e_masks = self.model.encode_edus((e_input_words, e_input_poses, e_input_masks, e_boundaries, e_txt)
