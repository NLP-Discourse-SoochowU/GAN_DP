...
class PartitionPtr(nn.Module):
    def __init__(self, hidden_size, dropout, word_vocab, pos_vocab, nuc_label, rel_label,
        ...
        self.input_dense = nn.Linear(XLNET_SIZE, hidden_size)
        # component
    ...
    def encode_edus(self, e_inputs, tokenizer_xl=None, model_xl=None):
        if EDU_ENCODE_V == 0:
            return self.encode_edus1(e_inputs)
        else:
            return self.encode_edus2(e_inputs, tokenizer_xl, model_xl)

    def encode_edus1(self, e_inputs):
        e_input_words, e_input_poses, e_masks, _, _ = e_inputs
    ...
    def encode_edus2(self, e_inputs, tokenizer_xl=None, model_xl=None):
        e_input_words, e_input_poses, e_masks, _, e_txt = e_inputs
        batch_size, max_edu_seqlen, max_word_seqlen = e_input_words.size()
        e_x = torch.zeros([1, XLNET_SIZE]).float().cuda()  # pad
        edu_encoded_x = None
        for e_in_edus in e_txt:
            ...
            while idx < len(e_in_edus):
                ...
                idx += 1
            result_edu_s = torch.tensor([result_edu_s]).cuda()  # (1, 6)
            ...
            if outputs is None:
                input("wrong")
            e_rep_idx = torch.Tensor(e_rep_idx).long().cuda()
            outputs = torch.index_select(outputs, 0, e_rep_idx)
            while idx < max_edu_seqlen:
                idx += 1
        ...
        edu_encoded = edu_encoded_x.view(batch_size, max_edu_seqlen, self.edu_encoder.output_size)
        e_masks = (e_masks.sum(-1) > 0).int()
        return edu_encoded, e_masks
    ...
    def loss(self, e_inputs, d_inputs, grounds, n_epoch, tokenizer_xl=None, model_xl=None):
        e_inputs_, e_masks = self.encode_edus(e_inputs, tokenizer_xl, model_xl)
